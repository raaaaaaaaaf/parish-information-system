...
const TABLE_HEAD = [
  ...
  { id: 'fullname', label: 'Bride', alignRight: false },
  { id: 'fullname1', label: 'Bridegroom', alignRight: false },
...
export default function MarriageContactPage() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const docsQuery = query(collection(db, "docsData"), where("docType", "==", "Marriage"))
  ...
  return (
    <>
      <Helmet>
        <title> Marriage Contract Records | Birhen Del Carmen Online Parish Information System   </title>
      </Helmet>

      <Container>
        <Stack direction="row" alignItems="center" justifyContent="space-between" mb={5}>
          <Typography variant="h4" gutterBottom>
            Marriage Contract Records
          </Typography>
        </Stack>
                  ...
                  {docsList.slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage).map((docs, index) => {
                    ...
                    return (
                      <TableRow hover key={index} tabIndex={-1} role="checkbox" selected={selectedUser}>
                        ...
                        <TableCell align="left">{docs.fullName1}</TableCell>

                        <TableCell align="left">{docs.address}</TableCell>
                        ...
                        <TableCell align="left">
                          <Link to={`editmarriage/${docs.id}`} style={{ textDecoration: 'none', color: 'black'}}>
                          <IconButton size="large" color="inherit" onClick={() => setDocId(docs.id)}>
                            <Iconify icon={'material-symbols:edit-outline'} />
                          </IconButton>
                          </Link>

                          <IconButton size="large" color="inherit" onClick={() => deleteMarriage(docs.id)}>
                            <Iconify icon={'material-symbols:delete-outline'} />
                          </IconButton>

                          <Link to={`viewmarriage/${docs.id}`} style={{ textDecoration: 'none', color: 'black'}}>
